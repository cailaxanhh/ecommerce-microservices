...
**Scope:** User Service, Product Service, Order Service, Payment Service, Inventory Service, Notification Service (each owns its own data)
...
│  (sync REST)     │  │  (sync REST)       │  │  (event-driven)     │
│                  │  │                    │  │                     │
│ • create user    │  │ • list products    │  │ • REST API          │
│ • get profile    │  │ • get product      │  │ • create order      │
│ • manage address │  │ • price lookup     │  │ • check status      │
│                  │  │ • validate exists  │  │                     │
...
└──────────────────┘  └────────────────────┘  │ Outbox Table        │
                            ...
                            │                            │                     │
                    ┌───────▼────────┐      ┌───────────▼──────┐   ┌───────────▼──┐
                    │  order-events  │      │ payment-events   │   │inventory-evt │
        ...
        │                   │                  │    │                     │
┌───────▼────────┐  ┌──────▼──────────────┐ ┌─▼───▼────────────┐ ┌──────▼──────────┐
│ NOTIFICATION   │  │ PAYMENT SERVICE    │ │ INVENTORY SERVICE │ │ BACKGROUND      │
│ SERVICE        │  │ (sync + async)     │ │ (async)           │ │ WORKER          │
│                │  │                    │ │                   │ │                 │
│ Event Consumer │  │ REST/Event API     │ │ Event Consumer    │ │ Job Queue       │
│                │  │ • charge card      │ │ • reserve stock   │ │ (BullMQ/Kafka)  │
│ Notif Log DB   │  │ • refund           │ │ • release stock   │ │                 │
│ (Postgres)     │  │                    │ │                   │ │ Lambda/Compute  │
│                │  │ Payment DB         │ │ Inventory DB      │ │ (Separate)      │
│ Send emails,   │  │ (Postgres)         │ │ (Postgres)        │ │                 │
│ SMS, push      │  │ • payments         │ │ • stock           │ │ PDF generation  │
│                │  │ • ledger           │ │ • reservations    │ │ Data aggregation│
│ mTLS enabled   │  │ • outbox           │ │ • outbox          │ │                 │
│                │  │                    │ │                   │ │                 │
└────────────────┘  │ Outbox Table       │ │ mTLS enabled      │ └─────────────────┘
                    │ (publishes)        │ │                   │
                    │ to Kafka           │ └───────────────────┘
                    │                    │
                    │ mTLS enabled       │
...
COMMUNICATION PATTERNS:
  ├─ User Service: SYNCHRONOUS (REST) — must be real-time
  ├─ Product Service: SYNCHRONOUS (REST) — must be real-time
  ├─ Order → Payment → Inventory → Notification: ASYNCHRONOUS (Kafka events)
  └─ Background Worker: ASYNC via Job Queue
```

**Key architectural decisions:**
- **User Service** — source of truth for user profiles, addresses, authentication state. Called synchronously when creating orders or processing payments.
- **Product Service** — source of truth for product catalog, prices, SKUs. Called synchronously for validation and lookups. Read-heavy, heavily cached in Redis.
...
- **Transactional Outbox** in every event-publishing service — guarantees events are published reliably without dual-write bugs.
...
- **Synchronous reads, asynchronous writes** — User and Product Services are read via REST (sync, must be current), but async events propagate through Order/Payment/Inventory (eventual consistency).
...
*Visual reference:* See **`order-platform-architecture.drawio`** (open at [draw.io](https://app.diagrams.net)) for a clean diagram of the main data flow.
...
|---|---|---|---|
| **User** | User DB (users, addresses, phone, email) | Create/read user profiles, manage addresses, authentication state | Payment data, order data, product catalog |
| **Product** | Product DB (products, SKUs, prices, descriptions) | List products, get product details, price lookups, product existence validation | Orders, payments, inventory, users |
| **Order** | Order DB (orders, order_items, outbox) | Create/cancel orders, hold "official" order status, validate products/users/addresses before creating | Payment info, stock counts, product details (reads via Product Service), user details (reads via User Service) |
| **Payment** | Payment DB (payments, ledger, outbox) | Charge/refund cards, tell everyone if payment worked | Order status, inventory, user accounts |
| **Inventory** | Inventory DB (stock, reservations, outbox) | Reserve/release stock, prevent overselling, validate products exist | Payment, order status, user data |
| **Notification** | Notification log (delivery records) | Consume domain events, send email/SMS/push, track delivery status | Any core business data |

**Synchronous vs Asynchronous calls:**
- **User Service**: Order/Payment/Notification call it synchronously (read user profile, address, contact info) — user data must be current
- **Product Service**: Order/Inventory call it synchronously (validate product exists, get current price) — product catalog must be current, read-heavy so heavily cached in Redis
- **Other services**: communicate via Kafka events (Order → Payment → Inventory → Notification) — async, eventual consistency
...
| Payment | **ABAC** | *(attribute-driven, not role-driven)* | Rules are more complex — e.g., "a support agent can view a payment only for customers in their region," or "refunds above a limit need a second approver." Roles alone can't express that without creating a huge, messy list of roles. |
...
- **mTLS between services**: on top of the ticket proving *who the user is*, mTLS lets services prove *which service* is calling — similar to how HTTPS proves a website is legit, but here it's service-to-service. This stops an attacker who's somehow gotten onto the internal network from just replaying a stolen ticket.
